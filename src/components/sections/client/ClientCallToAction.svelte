<script lang="ts">
  ...
  interface ClientCtaTranslations {
    title: string;
    subtitle: string;
    install: string;
    copied: string;
  ...
  interface Props {
    translations: ClientCtaTranslations;
  ...
  let {
    ...
    docsUrl = 'https://github.com/hamicek/noex-client#readme',
...
<section
  ...
  aria-labelledby="client-cta-title"
>
  ...
  <div class="content-wrapper max-w-5xl mx-auto relative z-10 px-4">
    ...
    <!-- Action Buttons -->
    <div
      class="action-buttons flex flex-col sm:flex-row gap-4 justify-center mb-6 md:mb-8"
      class:visible={isVisible}
    >
      <a
        href={docsUrl}
    ...
    <!-- Peer Note -->
    <div class="peer-note" class:visible={isVisible}>
      <p class="peer-note-text">{translations.peerNote}</p>
    </div>
