<script lang="ts">
  ...
  interface ClientHeroTranslations {
    headline: string;
    subheadline: string;
    install: string;
    github: string;
  ...
  interface Props {
    translations: ClientHeroTranslations;
    githubUrl?: string;
  }

  let {
    translations,
    githubUrl = 'https://github.com/hamicek/noex-client',
  }: Props = $props();

  let isVisible = $state(false);
  let copied = $state(false);

  const installCommand = 'npm install @hamicek/noex-client';
  ...
  function scrollToFeatures() {
    const section = document.getElementById('client-features');
    if (section) {
...
<section id="client-hero" class="hero-section snap-section bg-bg-primary">
  ...
  <div class="code-background" aria-hidden="true">
    <pre class="floating-code"><code><span class="code-keyword">const</span> client = <span class="code-keyword">new</span> <span class="code-class">NoexClient</span>(url, {'{'}</code>
<code>  <span class="code-property">auth</span>: {'{'} <span class="code-property">token</span> {'}'},</code>
<code>  <span class="code-property">reconnect</span>: <span class="code-keyword">true</span>,</code>
<code>{'}'});</code>
<code><span class="code-keyword">await</span> client.<span class="code-method">connect</span>();</code></pre>
  </div>
  ...
  <div class="hero-content max-w-4xl mx-auto text-center relative z-10" class:visible={isVisible}>
    ...
    <h1 class="text-5xl md:text-7xl font-bold mb-6">
      <span class="logo-text">noex-client</span>
    </h1>
    ...
    <div class="cta-buttons flex flex-col sm:flex-row gap-4 justify-center mt-6">
      <a
        ...
        class="cta-secondary px-8 py-3 border border-border text-text-primary rounded-lg hover:border-accent-client hover:text-accent-client transition-all"
      >
