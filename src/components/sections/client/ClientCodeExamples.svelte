...
await client.rules.deleteFact('user:1:tier');`,
  ...
  function highlightCode(code: string): string {
    const keywords = new Set(['import', 'from', 'export', 'const', 'let', 'return', 'new', 'await', 'async', 'function', 'class', 'interface', 'true', 'false', 'null']);
    const types = new Set(['NoexClient', 'User']);
    const methods = new Set(['connect', 'disconnect', 'bucket', 'subscribe', 'unsubscribe', 'insert', 'get', 'update', 'delete', 'where', 'findOne', 'paginate', 'count', 'avg', 'transaction', 'emit', 'setFact', 'getFact', 'queryFacts', 'deleteFact']);
