<script lang="ts">
  ...
  interface StoreHeroTranslations {
    headline: string;
    subheadline: string;
    install: string;
    github: string;
  ...
  interface Props {
    translations: StoreHeroTranslations;
    githubUrl?: string;
  }

  let {
    translations,
    githubUrl = 'https://github.com/hamicek/noex-store',
  }: Props = $props();

  let isVisible = $state(false);
  let copied = $state(false);

  const installCommand = 'npm install @hamicek/noex-store';
  ...
  function scrollToFeatures() {
    const section = document.getElementById('store-features');
    if (section) {
...
<section id="store-hero" class="hero-section snap-section bg-bg-primary">
  ...
  <div class="code-background" aria-hidden="true">
    <pre class="floating-code"><code><span class="code-keyword">await</span> store.<span class="code-method">defineBucket</span>(<span class="code-string">'users'</span>, {'{'}</code>
<code>  <span class="code-property">key</span>: <span class="code-string">'id'</span>,</code>
<code>  <span class="code-property">schema</span>: {'{'}</code>
<code>    <span class="code-property">id</span>: {'{'} <span class="code-property">type</span>: <span class="code-string">'string'</span>, <span class="code-property">generated</span>: <span class="code-string">'uuid'</span> {'}'},</code>
<code>    <span class="code-property">name</span>: {'{'} <span class="code-property">type</span>: <span class="code-string">'string'</span> {'}'},</code>
<code>  {'}'}</code>
<code>{'}'});</code></pre>
  </div>
  ...
  <div class="hero-content max-w-4xl mx-auto text-center relative z-10" class:visible={isVisible}>
    ...
    <h1 class="text-5xl md:text-7xl font-bold mb-6">
      <span class="logo-text">noex-store</span>
    </h1>
...
<style>
  ...
  /* Glowing Orbs — cyan-tinted for store */
  .glow-orb {
    position: absolute;
    border-radius: 50%;
    filter: blur(80px);
